...
def purge_yaml(data):
        ...
        basic_types = [int, float, basestring, list]
...
def main():
    ...
    # DELETE
        ...
        while cur_stack.stack_status == "DELETE_IN_PROGRESS":
            ...
            print(bcolors.HEADER + "[STACK DELETE]==>" +
                  bcolors.FAIL + "In progress [" + SPINNER[idx] + "][" + elapsed_time_string + "]" + bcolors.ENDC, end="\r")
            stdout.flush()
    ...
    # STATUS
        ...
        if args.monitor == 'False':
            ...
            while cur_stack.stack_status != "CREATE_FAILED" and cur_stack.stack_status != "CREATE_COMPLETE":
                ...
                elapsed_time_string = strftime("%H:%M:%S", gmtime(elapsed_time))
                print(bcolors.HEADER + "[STACK STATUS]==>" + bcolors.WARNING + "{} [".format(
                      cur_stack.stack_status) + SPINNER[idx] + "][" + elapsed_time_string + "]" + bcolors.ENDC, end="\r")
                stdout.flush()
                idx = (idx + 1) % len(SPINNER)
                sleep(1)
            print(bcolors.HEADER + "[STACK STATUS]==>" + bcolors.WARNING + "{} [".format(
